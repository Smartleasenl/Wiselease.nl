...
export default function BlogBeheer() {
  ...
  const GRADIENT_MAP: Record<string, string> = {
    'Advies': 'from-teal-500 to-cyan-500', 'Fiscaal': 'from-blue-500 to-indigo-500',
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {toast && (
        <div className={`fixed top-5 right-5 z-50 flex items-center gap-3 px-5 py-3 rounded-xl shadow-xl text-white text-sm font-semibold ${toast.type === 'success' ? 'bg-smartlease-teal' : 'bg-red-500'}`}>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            <div className="col-span-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <input value={form.slug} onChange={e => setForm(p => ({ ...p, slug: e.target.value }))}
                  className="flex-1 border border-gray-200 rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-smartlease-teal/30 focus:border-smartlease-teal"
          ...
          <div className="mb-4">
            ...
            <textarea value={form.excerpt} onChange={e => setForm(p => ({ ...p, excerpt: e.target.value }))}
              rows={3} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-smartlease-teal/30 focus:border-smartlease-teal resize-none"
          ...
          <div className="mb-4">
            ...
            <textarea value={form.content} onChange={e => setForm(p => ({ ...p, content: e.target.value }))}
              rows={8} className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-smartlease-teal/30 focus:border-smartlease-teal resize-none"
          ...
          <div className="mb-4">
            ...
            <div className="flex gap-2">
              ...
              <input value={form.image_url} onChange={e => setForm(p => ({ ...p, image_url: e.target.value }))}
                className="flex-1 border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-smartlease-teal/30 focus:border-smartlease-teal"
              ...
              <div key={post.id} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50/50 transition group">
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition flex-shrink-0">
                  <button onClick={() => togglePublished(post)} title={post.is_published ? 'Verbergen' : 'Publiceren'}
                    ...
                    {post.is_published ? <Eye className="h-4 w-4 text-smartlease-teal" /> : <EyeOff className="h-4 w-4 text-gray-400" />}
                  </button>
