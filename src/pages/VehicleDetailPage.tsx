...
function SpecItem({
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-3.5 rounded-xl bg-gray-50/70 border border-gray-100/80">
      <div className="text-smartlease-yellow flex-shrink-0">{icon}</div>
...
export function VehicleDetailPage() {
  ...
  // Fetch via Edge Function → krijgt images[] en opties[] mee
  useEffect(() => {
    if (!id) return;
    setLoading(true);
  ...
  // Build proxied image URLs from images array
  ...
  const nextImage = useCallback(() => {
    if (images.length > 1) goToImage((currentImageIndex + 1) % images.length);
  }, [currentImageIndex, images.length, goToImage]);

  const prevImage = useCallback(() => {
    if (images.length > 1) goToImage((currentImageIndex - 1 + images.length) % images.length);
  }, [currentImageIndex, images.length, goToImage]);
  ...
  const handleOfferteNavigate = () => {
    if (!vehicle) return;
    navigate('/offerte', {
      state: { vehicle: buildVehicleState(), calculator: calculatorState, cachedImageUrl: getCachedImageUrl() },
    });
  };
  ...
  const motorinhoud = (vehicle as any).motorinhoud as string | undefined;
                ...
                {images.length > 0 ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-gray-400">
                    <div className="text-center">
                      ...
                      <p className="text-sm">{vehicle.merk} {vehicle.model}</p>
                    </div>
                  </div>
                )}
            ...
            <div className="animate-fade-up opacity-0 delay-4 bg-white rounded-2xl shadow-sm px-5 md:px-7">
              <AccordionSection title="Informatie" defaultOpen={true}>
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {motorinhoud && (
                    <SpecItem icon={<Settings className="h-[18px] w-[18px]" />} label="Cilinderinhoud" value={motorinhoud} />
                  )}
                  ...
                  {nap && (
                    <SpecItem icon={<Check className="h-[18px] w-[18px]" />} label="NAP" value="Ja" />
                  )}
                </div>
              </AccordionSection>
