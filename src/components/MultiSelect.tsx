      ...
      <button
        ...
        className={`w-full px-3 py-2.5 border rounded-xl text-sm text-left transition-all flex items-center justify-between ${
          disabled
            ...
            : isOpen
            ? 'border-smartlease-yellow ring-2 ring-smartlease-yellow bg-white'
      ...
      {selected.length > 1 && (
        <div className="flex flex-wrap gap-1 mt-1.5">
          {selected.map((item) => (
            <span
              key={item}
              className="inline-flex items-center bg-smartlease-yellow/10 text-smartlease-yellow text-xs font-medium pl-2 pr-1 py-0.5 rounded-full"
            >
              {item}
              <button onClick={(e) => removeOption(item, e)} className="ml-0.5 p-0.5 hover:bg-smartlease-yellow/20 rounded-full">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
            ...
            <div className="p-2 border-b border-gray-100">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Zoeken..."
                  className="w-full pl-8 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-smartlease-yellow focus:border-smartlease-yellow outline-none"
                />
              </div>
            </div>
          )}
              ...
              filteredOptions.map((option) => (
                ...
                >
                  <input
                    ...
                    className="w-4 h-4 text-smartlease-yellow border-gray-300 rounded focus:ring-smartlease-yellow flex-shrink-0"
                  />
          ...
          {selected.length > 0 && (
            <div className="border-t border-gray-100 px-3 py-2">
              <button
                ...
                className="text-xs text-gray-400 hover:text-smartlease-yellow transition font-medium"
              >
                Selectie wissen
              </button>
            </div>
